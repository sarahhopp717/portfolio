...
<nav class="flex items-center justify-between px-6 py-4 bg-black text-white">
  ...
  <div class="hidden md:flex gap-6 items-center">
    ...
    <a use:route href="/gear">Gear & Programs</a>

    <div
      class="relative"
      onmouseenter={() => (bandsOpen = true)}
      onmouseleave={() => (bandsOpen = false)}
    >
      <button class="flex items-center gap-1">
        Bands
      </button>
      {#if bandsOpen}
        <div class="absolute top-full left-0 flex flex-col bg-black border border-gray-700 rounded min-w-48 z-10">
          {#each flyers as f, i}
            <a use:route href="/bands" onclick={() => goToFlyer(i)} class="px-4 py-2 hover:bg-gray-800">
              {f.genre}
            </a>
          {/each}
        </div>
...
<div class="md:hidden flex flex-col gap-2 px-6 py-4 bg-black text-white">
  ...
  <a use:route href="/gear" onclick={() => (menuOpen = false)}>Gear & Programs</a>
